...
mod tests {
    ...
    const TREE_SITTER_RUST: &str = "https://github.com/tree-sitter/tree-sitter-rust/releases/download/v0.24.0/tree-sitter-rust.wasm";

    #[tokio::test]
    async fn test_snips() -> anyhow::Result<()> {
        // let langspec = include_str!("../etc/languages.yml");
        let langspec = dedent(&format!(
            r#"
          rust:
            extensions:
                - rs
            grammar_path: {TREE_SITTER_RUST}
            queries:
                functions: |
                    (function_item
        ...
        let first = &item.captures[0];

        assert_that!(first.node.utf8_text(source_code)?, starts_with("fn"));

        let second = &item.captures[1];
        assert_that!(second.node.utf8_text(source_code)?, starts_with("foobar"));
        Ok(())
    }
}
